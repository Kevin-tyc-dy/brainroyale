...
// 先連 MySQL 建表，再啟動 HTTP server
initDB()
  .then(() => {
    server.listen(PORT, () => {
      console.log(`
╔═══════════════════════════════════════════╗
║   🎮  戰學者──知識大挑戰 · BrainRoyale Server    ║
║                                           ║
║   學生遊戲  →  http://localhost:${PORT}      ║
║   教師控制  →  http://localhost:${PORT}/teacher ║
╚═══════════════════════════════════════════╝
      `);
    });
  })
  .catch(err => {
    console.error('[MySQL] 啟動失敗，無法連接資料庫：', err.message);
    process.exit(1);
  });
